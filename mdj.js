...
const coke = '米读极速版'
    ...
    let status, no;
status = (status = ($.getval("mdjstatus") || "1")) > 1 ? `${status}` : ""; // 账号扩展字符
const mdjurlArr = [], mdjArr = []
let mdjurl = $.getdata('mdjurl')
    let mdj = $.getdata('mdj')
...
if ($.isNode()) {
  ...
  for (let i = 2; i <= fqxscount; i++) {
    ...
    console.log(`------------- 共${mdjurlArr.length}个账号----------------\n`)
    for (let i = 0; i < mdjArr.length; i++) {
        if (mdjArr[i]) {
            message = ''
                note = ''
                mdjurl = mdjurlArr[i];
